...
GuessTheEmotion::GuessTheEmotion(boost::shared_ptr<AL::ALBroker> broker, const std::string& name)
    ...
    mutex(AL::ALMutex::createALMutex())
{
    ...
    functionName("postSay", getName(), "Says the given phrase using a post-call, meaning the speech thread can run concurrently");
    addParam("phrase", "");
    BIND_METHOD(GuessTheEmotion::postSay);
...
void GuessTheEmotion::postSay(const std::string phrase)
{
    speech.post.say(phrase);
}
